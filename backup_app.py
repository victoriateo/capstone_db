    ...
    html.Div([
        ...
        html.Div(
            [
                ...
                # Bottlenecks graph- hard-coded for now
                html.Div([
...
def filter_by_city(city):
    ...
    if city == 'All Cities':  # for 'All Cities'
        ...
        new_df = mdf[mdf['start_city'] == city]
    return new_df
...
@app.callback(Output('stations-by-city-pie', 'figure'),
              [Input('time-dropdown', 'value'),
               Input('city-dropdown', 'value')])
def make_pie_figure(time, city):
    layout_pie = copy.deepcopy(layout)
    dff = filter_dataframe(time, city)
    print(make_pie_figure)
    print(city)
    if city == 'All Cities':
        ...
        pop_start_stn = dff[0]['start_station_name'].value_counts().to_dict(into=OrderedDict)
        print(pop_start_stn)
        figure = go.Figure(data=[go.Table(columnwidth = [400,150],
                                          header=dict(values=['Start Station Name', 'Number of Trips']),
                         cells=dict(values= [list(pop_start_stn.keys()),list(pop_start_stn.values())],
                                    align='center'))])
        figure.update_layout(width=550, height=480)
        return figure
